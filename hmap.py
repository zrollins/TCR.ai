...
#MART1, 140ns Occupancy
hb = gromacs.formats.XPM("/Users/zrollins/Documents/Documents/DMF5_MART1/rndm/bio/hCD9/140/hmap.xpm", reverse=True)
hmap = hb.array
...
#High Occupancy
h, d, a= [],[],[]
with open("/Users/zrollins/Documents/Documents/DMF5_MART1/rndm/bio/hCD9/140/h.log") as f:
    for line in f:
        if line.startswith('#'):
            continue
        if line.startswith('@'):
...
#Atoms
h1,d1,a1=[],[],[]
with open("/Users/zrollins/Documents/Documents/DMF5_MART1/rndm/bio/hCD9/140/h.ndx") as f:
    for line in f:
        if line.startswith('['):
            continue
...
def label_donor (row):
    ...
    if row['Donor Atom'] >5983 and row['Donor Atom'] <= 6106:
        return 'hCD9_' + row['Donor']
    if row['Donor Atom'] >6106 and row['Donor Atom'] <= 9113:
        return 'TCRα_' + row['Donor']
    if row['Donor Atom'] >9113 and row['Donor Atom'] <= 12822:
...
def label_acceptor (row):
    ...
    if row['Acceptor Atom'] >5983 and row['Acceptor Atom'] <= 6106:
        return 'hCD9_' + row['Acceptor']
    if row['Acceptor Atom'] >6106 and row['Acceptor Atom'] <= 9113:
        return 'TCRα_' + row['Acceptor']
    if row['Acceptor Atom'] >9113 and row['Acceptor Atom'] <= 12822:
...
print(hmaps)
hmaps.to_csv('/Users/zrollins/Documents/Documents/DMF5_MART1/rndm/bio/hCD9/140/hmap.csv', header=None)
